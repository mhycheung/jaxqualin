...
def qnm_fit_func(t, qnm_fixed_list, fix_mode_params_list, free_mode_params_list, part = None):
    Q = 0
    for qnm_fixed, fix_mode_params in zip(qnm_fixed_list, fix_mode_params_list):
...
def qnm_fit_func_wrapper_complex(t, qnm_fixed_list, N_free, *args):
    ...
    t_real = t[0::2]
    t_imag = t[1::2]
    h_real = qnm_fit_func_wrapper(t_real, qnm_fixed_list, N_free, *args, part = "real")
    h_imag = qnm_fit_func_wrapper(t_imag, qnm_fixed_list, N_free, *args, part = "imag")
...
class QNMFit:
    ...
    def __init__(self, h, t0, N_free, qnm_fixed_list = [], jcf = CurveFit(), params0 = None, max_nfev = 100000):
        self.h = h
        self.t0 = t0
        self.N_free = N_free
    ...
    def do_fit(self):
        ...
        self.popt, self.pcov = self.jcf.curve_fit(lambda t, *params: qnm_fit_func_wrapper_complex(t, self.qnm_fixed_list,
                                                                                              self.N_free, params), 
                                             np.array(self._time_interweave), np.array(self._h_interweave), 
                                             p0 = self.params0, max_nfev = self.max_nfev)
        self.reconstruct_h = qnm_fit_func_wrapper(self.time, self.qnm_fixed_list, self.N_free, self.popt)
        self.h_true = self.hr + 1.j*self.hi
        self.mismatch =1-(np.abs(np.vdot(self.h_true,self.reconstruct_h)/(np.linalg.norm(self.h_true)*np.linalg.norm(self.reconstruct_h))))
...
class QNMFitVaryingStartingTimeResult:
    ...
    def process_results(self):
        ...
        self.results_dict = {**self.A_fix_dict, **self.A_free_dict, **self.phi_fix_dict, **self.phi_free_dict,
