...
class waveform:
    ...
    def postmerger(self, t0):
        ...
        return self.fulltime[startindx:]-self.peaktime, jnp.real(self.fullh[startindx:]), jnp.imag(self.fullh[startindx:])

def get_waveform_SXS(SXSnum, l, m, res = 0, N_ext = 2):
    catalog = sxs.catalog.Catalog.load()
    waveformloadname = catalog.select(f"SXS:BBH:{SXSnum}/Lev./rhOverM")[-1+res]
    metaloadname = catalog.select(f"SXS:BBH:{SXSnum}/Lev./metadata.json")[-1+res]
    ...
    indx = hs.index(l, m)
    h = waveform(hs[:,indx].time, hs[:,indx].real + 1.j*hs[:,indx].imag, l = l, m = m)
...
def waveformabsmax(time, hr, hi, startcut = 500):
    ...
    return maxindx, time[maxindx], np.array(np.sqrt(hr[maxindx]**2+hi[maxindx]**2))[0]
...
def get_relevant_lm_waveforms_SXS(SXSnum, tol = 1/40, prec = False, includem0 = False, res = 0, N_ext = 2):
    Mf, af, Level, hdict = get_SXS_waveform_dict(SXSnum, res = res, N_ext = N_ext)
    relevant_lm_list = getdommodes(hdict, tol = 1/40, prec = False, includem0 = False)
    waveform_dict = {}
