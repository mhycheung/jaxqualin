        ...
        if self.Schwarzschild:
            if not hasattr(self.params0, "__iter__"):
                self.params0 = np.array(
                    self.guess_fixed * self.N_fix + self.guess_free * self.N_free + self.guess_M_a[:1])
            if self.include_mirror:
            ...
            if not hasattr(self.params0, "__iter__"):
                self.params0 = np.array(
                    self.guess_fixed * self.N_fix + self.guess_free * self.N_free + self.guess_M_a)
            lower_bound = [-np.inf] * \
...
class QNMFitVaryingStartingTime:
    ...
    def __init__(
        ...
        if var_M_a:
            warnings.warn("var_M_a is not tested yet, proceed with caution", UserWarning)
            self.N_free = len(qnm_free_list)
            self.qnm_free_list = qnm_free_list
        else:
            self.N_free = N_free
        ...
        self.Schwarzschild = Schwarzschild
        if self.Schwarzschild:
            warnings.warn("Schwarzschild is not tested yet, proceed with caution", UserWarning)
    ...
    def initial_guesses(self, jcf: Optional[CurveFit] = None) -> Tuple[int, List[QNMFit], List[np.ndarray]]:
        """
    ...
    def do_fits(self, jcf: Optional[CurveFit]=None, return_jcf: bool=False) -> Optional[CurveFit]:
        """
        Perform the fits.
