...
_default_full_mode_data_path = os.path.join(DATA_SAVE_PATH, 'SXS_results_latest.csv')
_default_hyperfit_data_path = os.path.join(DATA_SAVE_PATH, 'hyperfit_functions_latest.json')
_default_interpolate_data_path = os.path.join(DATA_SAVE_PATH, 'interpolate_data_latest.json')
...
def download_file(filepath, url, overwrite = 'update'):
    ...
    if overwrite == 'force' or not os.path.exists(filepath):
        ...
        if last_modified_time(url) > time.gmtime(os.path.getmtime(filepath)):
            ...
            print('{} is not more recent than {}, file will not be downloaded.'.format(url, filepath))
    elif overwrite == 'never':
        pass
    else:
        raise ValueError("overwrite must be one of 'force', 'update', or 'never'")

    if download: 
        print("Downloading data file from {} to {}".format(url, filepath))
        urlretrieve(url, filepath)

def download_full_mode_data(filepath = _default_full_mode_data_path, url = _full_data_url, overwrite = 'update'):
    download_file(filepath, url, overwrite = overwrite)

def download_hyperfit_data(filepath = _default_hyperfit_data_path, url = _hyperfit_url, overwrite = 'update'):
    download_file(filepath, url, overwrite = overwrite)

def download_interpolate_data(filepath = _default_interpolate_data_path, url = _interpolate_url, overwrite = 'update'):
    download_file(filepath, url, overwrite = overwrite)
    ...
    if PN:
        def hyperfit_func(eta, chi_p, chi_m):
            ...
            for term_list in hyperfit_list:
                val += term_list[3]*eta**term_list[0]*chi_p**term_list[1]*chi_m**term_list[2]
            if var == 'A':
                if m % 2 == 0:
                    adj = eta
                else:
...
def make_hyper_fit_functions(filepath = _default_hyperfit_data_path, PN = True):
    ...
    for var in hyperfit_data:
        for mode in hyperfit_data[var]:
            ...
            hyperfit_func_dict[mode][var] = hyperfit_list_to_func(hyperfit_list, m, var = var, PN = PN)
...
def make_interpolators(filepath = _default_interpolate_data_path, PN = True):
    ...
    for mode in interpolate_data:
        interpolate_func_dict[mode] = make_hyper_interpolator(interpolate_data[mode], PN = PN)
