...
def _(mo):
    mo.md(r"""
    # Example: Fitting a real (linearly polarized) waveform

    In this notebook we demonstrate how to fit a real (linearly polarized) waveform with `jaxqualin`. 
    This is useful for fitting head-on mergers.

    1. **Free-frequency fit** across varying $t_0$
    2. **Fixed-frequency fit** across varying $t_0$
    3. **Variable-mass fit** across varying $t_0$ with `real=True` to recover $M$
    """)
    return
...
def _(mo):
    mo.md(r"""
    ## Build a toy real waveform

    We construct a real waveform with two QNMs at $a = 0$: the $(2,2,0)$ fundamental mode and the $(3,2,0)$ mode.
...
def _(delayed_QNM, mode_list, np, waveform):
    ...
    # Take the real part for a real waveform
...
def _(h_schw, np, plt):
    ...
    _ax.set_title('Toy real waveform (real part)')
    _fig
    return
...
def _(mo):
    ...
    First we fit with free QNMs ($N_f = 2$) whose frequencies are not fixed, to see if the resulting frequencies converge to the expected QNMs for this real waveform.
...
def _(plot_amplitudes, plot_phases, plt, result_schw_fixed_vst, schw_modes):
    ...
    _fig.suptitle('Real-waveform fixed-frequency fit across $t_0$')
    _fig.tight_layout()
    _fig
    return
...
def _(mo):
    ...
    We use the VarMa (varying mass and spin) fitter across starting times with `real=True` to see how the recovered mass $M$ evolves with $t_0$.
    The spin will not be recovered, since we assumed that the merger is head-on, which produces a Schwarzschild black hole remnant.
...
def _(Mf_true, np, plt, result_schw_varma_vst, schw_varma_t0_arr):
    ...
    _ax.set_ylabel(r'$M$')
    _ax.set_title('Recovered mass across starting times (real waveform)')
    _ax.legend()
    _fig.tight_layout()
    _fig
    return
